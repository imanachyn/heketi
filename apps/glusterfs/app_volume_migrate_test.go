...
func TestVolumeMigrate(t *testing.T) {
	...
	*vc = *v
	...
	r, err = http.Post(ts.URL+"/nodes/"+info.Bricks[0].NodeId+"/state", "application/json", bytes.NewBuffer(request))
	tests.Assert(t, err == nil)
...
func TestReplicateVolumeMigrate(t *testing.T) {
	tmpfile := tests.Tempfile()
	defer os.Remove(tmpfile)
	...
	*vc = *v
	...
	tests.Assert(t, info.Durability.Type == api.DurabilityReplicate)
	tests.Assert(t, info.Durability.Replicate.Replica == 2)
}
